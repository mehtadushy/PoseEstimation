...
int main(int argc, char *argv[])
{
    ...
    //To Do: When implementing the useInit functionality, initialize x with the incoming
    //estimates
    ...
    * Eigen::AngleAxisf(x(5), Vector3f::UnitX());
    ...
    //Eigen::NumericalDiff<SolvePnPFunctor> numDiff(the_functor);
    Eigen::LevenbergMarquardt<SolvePnPFunctor,double> lm(the_functor);
    int ret = lm.minimize(x);
    //Check if minimization was properly done. If yes, then form the output matrices and flag updates
